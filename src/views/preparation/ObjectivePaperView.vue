    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div class="space-y-2">
          <div class="flex gap-2">
            <button @click="generateWithAI('paper')" :disabled="aiLoading" class="btn-primary flex-1 justify-center">
              <Sparkles class="w-4 h-4" /> {{ aiLoading && aiMode === 'paper' ? 'Generating…' : 'AI Paper' }}
            </button>
          ...
          <div class="flex gap-2">
            <button @click="generatePaper" :disabled="aiLoading" class="btn-secondary flex-1 justify-center text-xs">
      ...
      <div class="lg:col-span-2 space-y-4">
        ...
        <div v-if="aiLoading" class="card p-16 flex flex-col items-center" style="color:var(--t-text3)">
          ...
          <p class="text-sm">{{ boardName }} paper setter is generating your {{ aiMode === 'predicted' ? 'predicted' : '' }} questions…</p>
          <p class="text-xs mt-1">Running llama3 locally — this can take a moment.</p>
        </div>
...
import { generateObjectiveQuestions, generatePredictedPaper } from '@/services/ollamaService'
...
async function generateWithAI(mode) {
  ...
  if (qs.length) {
    ...
    sourceNote.value = `Generated by llama3 to the ${boardName.value} ${pattern.value.label} pattern.`
  } else {
