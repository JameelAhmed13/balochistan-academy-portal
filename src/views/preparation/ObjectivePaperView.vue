...
import { generateObjectiveQuestions, generatePredictedPaper, ollamaConfig, getLastEngine } from '@/services/ollamaService'
...
async function generateWithAI(mode) {
  ...
  try {
    if (mode === 'predicted') {
      ...
      let examples = ''
      try { examples = await realContextSnippet({ grade: auth.user?.gradeCode || 9, subject: subject.value.name, sample: 6 }) } catch { examples = '' }
  ...
  if (qs.length) {
    questions.value = qs
    const eng = getLastEngine() === 'gemini' ? 'Gemini' : ollamaConfig.MODEL
    sourceBadge.value = { text: mode === 'predicted' ? '🔮 AI Predicted' : '✨ AI Generated', cls: 'badge-purple' }
    sourceNote.value = `Generated by ${eng}, grounded on real ${boardName.value} Grade ${auth.user?.gradeCode || 9} ${subject.value.name} questions.`
  } else {
    // Graceful fallback — serve REAL board questions (never dummy).
    let fb = []
    try { fb = await getRealQuestions({ grade: auth.user?.gradeCode || 9, subject: subject.value.name, limit: count }) } catch { fb = [] }
    if (fb.length) {
      questions.value = fb
      sourceBadge.value = { text: '📋 Real Board Questions', cls: 'badge-green' }
      sourceNote.value = `AI was unavailable — served real ${boardName.value} questions from the ingested bank instead.`
    } else {
      questions.value = content.getQuestions(+props.bookId, { ...filters.value, limit: count })
      sourceBadge.value = { text: 'Question Bank', cls: 'badge-indigo' }
      sourceNote.value = 'AI and the ingested bank were both unavailable for this subject — used the local bank.'
    }
  }
  generated.value = true
...
async function generateBoard() {
  ...
  const count = pattern.value.objective.count
  let qs = []
  try { qs = await getRealQuestions({ grade: auth.user?.gradeCode || 9, subject: subject.value.name, limit: count }) } catch { qs = [] }
  if (qs.length) {
    questions.value = qs
    sourceBadge.value = { text: '📋 Real Board Questions', cls: 'badge-green' }
    sourceNote.value = `Real ${boardName.value} questions for Grade ${auth.user?.gradeCode || 9} ${subject.value.name} — from the ingested assessment bank.`
  } else {
    questions.value = content.getQuestions(+props.bookId, { ...filters.value, limit: count })
    sourceBadge.value = { text: 'Question Bank', cls: 'badge-indigo' }
    sourceNote.value = `No ingested board bank for Grade ${auth.user?.gradeCode || 9} ${subject.value.name} yet — used the local bank.`
  }
  generated.value = true
  aiLoading.value = false
}
