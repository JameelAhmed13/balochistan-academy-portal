...
const kpiCards = ref([
  { label: 'Students',      value: '—', icon: '👥', color: '#8b5cf6', key: 'totalStudents' },
  { label: 'Tests Taken',   value: '—', icon: '📝', color: '#3b82f6', key: 'totalAttempts' },
  { label: 'Coins Issued',  value: '—', icon: '🪙', color: '#10b981', key: 'totalCoins' },
  { label: 'Questions',     value: '—', icon: '❓', color: '#a855f7', key: 'totalQuestions' },
  { label: 'Tests Created', value: '—', icon: '📅', color: '#f59e0b', key: 'totalTests' },
  { label: 'Grades',        value: '—', icon: '🎓', color: '#14b8a6', key: '_grades' },
])

onMounted(async () => {
  try {
    const s = (await api.get('/admin/analytics')).data
    kpiCards.value = kpiCards.value.map((c) => {
      const val = c.key === '_grades' ? (s.byGrade?.length ?? '—') : s[c.key]
      return { ...c, value: fmt(val) }
    })
