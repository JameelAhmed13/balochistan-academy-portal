      ...
      <div class="qb-modal qb-modal-wide">
        ...
        <div class="qb-modal-body">
          ...
          <template v-if="!aiResults.length">
            ...
            <p v-if="aiSuccessMsg" class="qb-success">{{ aiSuccessMsg }}</p>
...
function subjectName(id) { return id ? (catalog.findSubject(id)?.name ?? `Subject ${id}`) : '—' }
...
async function saveAIResults() {
  ...
  try {
    ...
    const saved = selectedAI.value.size
    aiResults.value  = []
    selectedAI.value = new Set()
    aiErrMsg.value   = ''
    aiSuccessMsg.value = `✅ ${saved} question${saved !== 1 ? 's' : ''} added to the bank successfully. Generate more or close.`
    load()
    loadStats()
  } catch (e) {
...
onMounted(() => {
  Promise.all([load(), loadStats(), catalog.fetchAllSubjects()])
})
</script>
...
.qb-success {
  margin:0; padding: 10px 14px; border-radius: 9px;
  font-size:0.83rem; background:#f0fdf4; border:1px solid #bbf7d0; color:#15803d;
}
.qb-spinner {
