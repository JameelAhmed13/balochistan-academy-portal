  ...
  <div class="animate-fade-in space-y-5" ref="resultEl">
    ...
    <div class="kua-grid">
      <div class="kua-card kua-blue">
        ...
        <div class="kua-lbl">Knowledge Score: {{ knowledgePct }}</div>
      </div>
      <div class="kua-card kua-green">
        <div class="kua-val">{{ understandingScore }}</div>
        <div class="kua-lbl">Understanding Score: {{ understandingPct }}</div>
      </div>
      <div class="kua-card kua-rose">
        <div class="kua-val">{{ applyingScore }}</div>
        <div class="kua-lbl">Applying Score: {{ applyingPct }}</div>
      </div>
    ...
    <div class="dtr-actions">
      ...
      <RouterLink to="/app/daily-tests/combined" class="dtr-btn-retry">Retry Challenge</RouterLink>
    </div>
...
function catScore(qs) { return qs.filter(q => q.userAnswer === q.correct).length + '/' + qs.length }
function catPct(qs) { if (!qs.length) return '-'; return Math.round(qs.filter(q => q.userAnswer === q.correct).length / qs.length * 100) + '%' }

const knowledgeScore = computed(() => catScore(knowledgeQs.value))
const knowledgePct = computed(() => catPct(knowledgeQs.value))
const understandingScore = computed(() => catScore(understandingQs.value))
...
.dtr-actions { display: flex; gap: 0.75rem; }
.dtr-btn-back { padding: 0.6rem 1.25rem; border-radius: 8px; border: 1px solid var(--t-border); background: var(--t-surface); color: var(--t-text2); font-size: 0.85rem; font-weight: 600; text-decoration: none; }
.dtr-btn-retry { padding: 0.6rem 1.5rem; border-radius: 8px; background: #4caf50; color: white; font-size: 0.85rem; font-weight: 700; text-decoration: none; }
</style>
