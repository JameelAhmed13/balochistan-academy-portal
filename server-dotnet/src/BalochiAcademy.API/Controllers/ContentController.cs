...
[Route("api/content")]
[Authorize]
public class ContentController(IApplicationDbContext db) : ControllerBase
{
    ...
    {
        var q = db.ContentItems.Where(c => c.IsPublished);
    ...
    {
        var q = db.VideoCourses.Include(c => c.Lessons).Where(c => c.IsPublished);
        if (!string.IsNullOrEmpty(gradeCode)) q = q.Where(c => c.GradeCode == gradeCode);
        if (subjectId.HasValue)               q = q.Where(c => c.SubjectId == subjectId);
    ...
    {
        var course = await db.VideoCourses
    ...
    {
        // inject ICurrentUserService via constructor is fine — using dynamic approach for brevity
        var userId = int.Parse(User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier)!.Value);
        return Ok(await db.StudentNotes.Where(n => n.UserId == userId)
    ...
    {
        req.UserId = int.Parse(User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier)!.Value);
        db.StudentNotes.Add(req);
        await db.SaveChangesAsync(ct);
    ...
    {
        var userId = int.Parse(User.FindFirst(System.Security.Claims.ClaimTypes.NameIdentifier)!.Value);
        var note = await db.StudentNotes.FirstOrDefaultAsync(n => n.Id == id && n.UserId == userId, ct);
