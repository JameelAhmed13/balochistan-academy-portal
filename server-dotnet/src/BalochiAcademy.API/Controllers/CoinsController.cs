...
[Route("api/coins")]
[Authorize]
public class CoinsController(IApplicationDbContext db, ICurrentUserService cu) : ControllerBase
{
    ...
    public async Task<IActionResult> Transactions([FromQuery] int page = 1, [FromQuery] int pageSize = 30, CancellationToken ct = default)
    {
        var q = db.CoinLedgers.Where(c => c.UserId == cu.UserId);
        var total = await q.CountAsync(ct);
        var items = await q.OrderByDescending(c => c.Timestamp)
            .Skip((page - 1) * pageSize).Take(pageSize)
            .Select(c => new CoinLedgerEntryDto(c.Id, c.Amount, c.Reason, c.Timestamp))
            .ToListAsync(ct);
    ...
    {
        var acct = await db.PayoutAccounts.FirstOrDefaultAsync(p => p.UserId == cu.UserId, ct);
        if (acct == null) return Ok((object?)null);
        return Ok(new PayoutAccountDto(acct.Id, acct.AccountName, acct.AccountNo, acct.Service));
    }
    ...
    public async Task<IActionResult> RequestWithdrawal([FromBody] CreateWithdrawalRequest req, CancellationToken ct)
    {
        ...
        await db.SaveChangesAsync(ct);
        return Created($"/api/coins/withdrawals/{wr.Id}",
            new WithdrawalRequestDto(wr.Id, wr.AmountCoins, wr.AmountPkr, wr.Status, wr.AdminNote, wr.CreatedAt, null));
    }
    ...
    public async Task<IActionResult> MyWithdrawals(CancellationToken ct)
    {
        var items = await db.WithdrawalRequests
            ...
            .Select(w => new WithdrawalRequestDto(w.Id, w.AmountCoins, w.AmountPkr, w.Status, w.AdminNote, w.CreatedAt, w.ProcessedAt))
            .ToListAsync(ct);
        return Ok(items);
    }
    ...
    public async Task<IActionResult> AdminWithdrawals([FromQuery] string? status, [FromQuery] int page = 1, [FromQuery] int pageSize = 20, CancellationToken ct = default)
    {
        var q = db.WithdrawalRequests.Include(w => w.User).AsQueryable();
    ...
    {
        var wr = await db.WithdrawalRequests.Include(w => w.User).FirstOrDefaultAsync(w => w.Id == id, ct);
